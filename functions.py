...
def validate_input(accepted_values, prompt):
    ...
    while not validated:
        ...
        for value in accepted_values:
            if value == input_to_be_validated:
                validated = True
                break
        if not validated:
            print("The input is in invalid.")
...
def make_appointment(barber_list, treatment_name_list, treatment_detail_list, free_slots):
    barber = validate_input(barber_list, "Who would you like to help you?")
...
# Check which time-slots are available looking at planned shifts and current appointments
...
# Looking at which time-slots are available considering the duration of the treatment of the customer
